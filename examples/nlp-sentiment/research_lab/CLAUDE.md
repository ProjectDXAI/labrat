...
5. Then invoke `/next` or run `python scripts/operator_helper.py next-prompt --runner claude --phase auto`.

## Autonomous operation within a single invocation

One turn is not one candidate. Within a single Claude Code invocation, the supervisor should:
...
- `/synthesize` — summarize recent evaluations before dispatching more work
...
Claude Code prompts for each bash invocation by default. For hands-off multi-turn operation, drop this exact file at `.claude/settings.json` in this lab's root. It allowlists the operator scripts and nothing else. Destructive operations still prompt.
