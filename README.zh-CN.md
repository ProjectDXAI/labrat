...
- **确定性工具，而不是凭记忆算数。** 十七个带版本、带 as-of 契约和闭式自检的方法。method 绑定会钉住实现版本，实现一旦变更，验证就会失败，直到有人重新核对卡片。

在内置种子上，纯相似度检索的命中率是满分 —— 但精确率只有 0.23、零反证，并且对**每一个**不适用的上下文都给出答案，包括一个早于其来源出版时间的历史情境。它作为对照臂保留在 lab 中。
