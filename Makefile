...
smoke-corpus: clean-smoke-corpus selftest
	...
	@echo ">>> Checking the rights gate holds at seed (only material we own may clear)..."
	@cd $(CORPUS_LAB) && $(PYTHON) scripts/corpus.py manifest > /dev/null && $(PYTHON) -c "import json; d=json.load(open('corpus/manifest.json')); assert d['include'], 'expected our own material to clear'; assert all(r['rights_status'] in {'owned_by_us','public_domain','cc0'} for r in d['include']), [r for r in d['include'] if r['rights_status'] not in {'owned_by_us','public_domain','cc0'}]; print(f'  gate OK: {d[\"include_count\"]} eligible entries, all ours; {d[\"hold_count\"]} third-party sources held')"
	...
	@cd $(CORPUS_LAB) && $(PYTHON) -c "import json, subprocess, sys; rows=json.loads(subprocess.run([sys.executable,'scripts/corpus.py','frontier','--limit','1','--json'],capture_output=True,text=True).stdout); assert rows, 'seed frontier is empty'; row=rows[0]; json.dump({'round':1,'entries':[{'title':'Smoke Fixture: A Companion Volume','authors':['Smoke Tester'],'year':2026,'bucket':row.get('suggested_bucket') or 'market_microstructure','form':'monograph','pages':120,'priority':5,'rights':{'status':'subscription_required','confidence':'confirmed','evidence':'https://example.invalid/terms','checked_at':'2026-01-01'},'resolves':[row['label']],'refs':[{'target':'zipkin-2000-foundations-inventory-management','relation':'syllabus_with'}]},{'id':'mit-ocw-6262-discrete-stochastic-processes','rights':{'status':'cc_by_nc_sa','confidence':'confirmed','evidence':'https://example.invalid/ocw','checked_at':'2026-01-01'}}]}, open('corpus/rounds/round-001/findings.yaml','w')); print('  fixture resolves: ' + row['label'][:58])"
	...
	@cd $(CORPUS_LAB) && $(PYTHON) scripts/corpus.py manifest | $(PYTHON) -c "import json,sys; d=json.load(sys.stdin); assert d['include']==2, f'expected our own material plus the newly-confirmed CC entry, got {d[\"include\"]}'; assert sorted(d['by_use_class']) == ['ingest_full','ingest_noncommercial'], d['by_use_class']; assert d['hold']>100, 'hold list looks wrong'; print(f'  manifest OK: {d[\"include\"]} included ({d[\"by_use_class\"]}), {d[\"hold\"]} held')"
