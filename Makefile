...
smoke-knowledge: clean-smoke-knowledge selftest
	...
	@cd $(KNOWLEDGE_LAB) && $(PYTHON) scripts/knowledge.py admission --json | $(PYTHON) -c "import json,sys; d=json.load(sys.stdin); assert d['considered']>0, d; assert d['rejected']>0, 'every source passing the admission gate means the gate is not checking anything'; row=[r for r in d['sources'] if not r['admitted']][0]; assert row['missing'], row; print(f'  admission OK: {d[\"admitted\"]} of {d[\"considered\"]} sources earned their place, {d[\"rejected\"]} are still records')"
