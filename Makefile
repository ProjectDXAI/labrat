...
smoke-knowledge: clean-smoke-knowledge selftest
	...
	@echo ">>> Checking a source has to earn its place before it counts..."
	@cd $(KNOWLEDGE_LAB) && $(PYTHON) scripts/knowledge.py admission --json | $(PYTHON) -c "import json,sys; d=json.load(sys.stdin); assert d['considered']>0, d; assert d['rejected']>0, 'every source passing means the gate is not checking anything'; nocard=[r for r in d['sources'] if r['stage']=='no_card_yet']; assert all(len(r['missing'])==1 for r in nocard), 'a source with no card must report one missing thing, not six'; print(f'  admission OK: {d[\"admitted\"]} compiled, {d[\"card_incomplete\"]} one artifact short, {d[\"no_card_yet\"]} not compiled')"
	...
	@echo ">>> Checking the analysis brief assembles and every passage carries a locator..."
	...
	@cd $(KNOWLEDGE_LAB) && $(PYTHON) -c "import sys; sys.path.insert(0,'scripts'); import passages; row={'passage_id':'x','entry_id':'e','file':'f.pdf','page':3,'use_class':'reference_only','rights_status':'all_rights_reserved','text':'held text '*40}; out=passages.emit_passage(row,{'held':40}); assert out['snippet'] and out['read_it_at']=='f.pdf p.3' and out['why_it_matched'], out; print('  passage OK: text emitted for anything held, always with a locator to check it against')"
