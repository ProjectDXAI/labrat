...
smoke-knowledge: clean-smoke-knowledge selftest
	...
	@cd $(KNOWLEDGE_LAB) && $(PYTHON) scripts/knowledge.py digest --context .digest-ctx.json --budget 999999 --arm always --abstain-threshold 0.01 | $(PYTHON) -c "import json,sys; d=json.load(sys.stdin); over=[r for r in d['passages'] if ['card','note','excerpt','unit'].index(r['tier'])>['card','note','excerpt','unit'].index(r['rights_ceiling'])]; assert not over, over; print(f'  ceiling OK: {len(d[\"passages\"])} anchors served, none above its licence even with an unbounded budget')"
