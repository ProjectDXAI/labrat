...
`--arm never` (cards only) and `--arm always` (everything the lab holds). If
...
# There is deliberately no rights gate in this module.
#
# `corpus.py` records a licence for every source, and that record is worth keeping: it is
# a fact about the source, and it is the thing to consult if any of this material is ever
# redistributed. But redistribution is not what this module does. The digest assembles
# text you already hold into a prompt on your own machine, and a licence restricting
# redistribution does not restrict reading. Gating local reading on it produced a tool
# that refused to show you a paper sitting on your own disk, which helped nobody.
#
# If output from this ever needs to be published, consult `corpus.py rights` at that
# point. That is where the question actually arises.
...
DEFAULT_DIGEST_POLICY: dict[str, Any] = {
    ...
    # tier the material can support. Weight is added to the anchor's value so the knapsack prefers to
    ...
    for card in cards:
        for raw in card.get("source_passage_ids") or []:
            ...
            options = _tier_options(
                card, unit, rows, all_rows, policy, full_text,
            ...
            candidates.append({
                ...
                "effective_floor": TIER_NAMES[floor],
                # A trigger asked for the text and the lab does not hold it. That is a
                # fetch instruction, not a policy decision.
                "starved": floor > best_available,
...
def _tier_options(
    ...
    if rows:
        ...
        if locator_resolved and len(joined) > len(excerpt):
            options.append({
                "tier": TIER_UNIT,
                "text": joined,
...
def compare_arms(
    ...
    return {
        ...
        "verdict": _verdict(agree / total, runs["policy"]["tokens_used"] / always_cost),
        "downgraded": [
...
def _verdict(match: float, cost_share: float) -> str:
    if cost_share >= 0.9:
    ...
    if served:
        ...
        for row in served:
            ...
            lines.append(
                ...
                f"{'read' if row['read'] else 'never opened'}"
            )
            lines.append("")
...
def self_test() -> dict[str, Any]:
    ...
    options = _tier_options({}, None, [], [], DEFAULT_DIGEST_POLICY, False)
    ...
    vague = _tier_options({}, {"notes": "n", "locator": "empirical section"}, rows, rows,
                          {**DEFAULT_DIGEST_POLICY, "excerpt_chars": 500},
    ...
    precise = _tier_options({}, {"notes": "n", "locator": "pp. 1-8"}, rows, rows,
                            tight, False, locator_resolved=True)
