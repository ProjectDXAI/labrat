...
3. Read the last ten entries in `state/evaluations.jsonl`. Look for structural patterns that the current families cannot express (for example: every family's best candidate keeps losing a decisive challenge along the same axis, suggesting the mutation axes are wrong for the research question).
